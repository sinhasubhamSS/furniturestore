...
const TrendingSection = () => {
  ...
  const renderContent = () => {
    ...
    if (error || !data) {
      return (
        <div className="w-full h-[170px] flex items-center justify-center">
          <p className="text-[var(--text-error)] font-semibold">
    ...
    return (
      <div
        className="flex gap-4 overflow-x-auto scrollbar-custom pb-2"
        role="list"
      ...
      >
        {data.map((product) => (
          <article
            ...
            className="flex-none w-[160px] md:w-[180px] bg-[var(--color-card)] border border-[var(--color-border-custom)] rounded-xl shadow-sm hover:shadow-lg hover:bg-[var(--color-hover-card)] transition-all duration-300 cursor-pointer outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent)] group"
            tabIndex={0}
            onKeyDown={(e) => {
          ...
          >
            {/* Image Container - Added padding to prevent corner touching */}
            <div className="w-full h-[130px] flex items-center justify-center bg-[var(--color-secondary)] rounded-t-xl overflow-hidden p-3">
              <img
  ...
  return (
    <section className="w-full py-8 border border-[var(--color-border-custom)] rounded-xl mt-8 bg-[var(--color-primary)] shadow-lg">
