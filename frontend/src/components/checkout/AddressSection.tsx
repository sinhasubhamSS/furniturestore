    ...
    try {
      ...
      setDeliveryStatus({
        ...
        message: isDeliverable
          ? "Delivery available"
          : response.deliveryInfo?.message || "Not deliverable",
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      if (watchedPincode && /^\d{6}$/.test(watchedPincode) && items.length) {
        const result = await checkDeliveryAndPricing(watchedPincode, items);

        // 🔥 IMPORTANT: Parent (CheckoutPage) ko notify karo
        onSelectionChange(
          result.deliverable,
          result.pricingData,
          watchedPincode,
        ...
        <div style={{ display: "grid", gap: 8 }}>
          {addresses.map((addr: Address) => (
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                <div
                  style={{ fontWeight: 700, color: "var(--color-foreground)" }}
                >
                  {addr.fullName}
                </div>
                <div
                ...
                {selectedId === addr._id && (
                  <div style={{ marginTop: 8 }}>
                    {deliveryStatus.checking ? (
                      ...
                      >
                        <div style={{ fontWeight: 700, color: "#256c3f" }}>
                          ✅ Deliverable
                        </div>
      ...
      {newAddress && (
        <form onSubmit={handleSubmit(onSubmit)} style={{ marginTop: 12 }}>
          ...
          >
            ...
            />
            <label htmlFor="isDefault" style={{ color: "var(--text-accent)" }}>
              Make default
            </label>
          </div>
          ...
          <div style={{ marginTop: 12 }}>
            <button
              type="submit"
              disabled={isSubmitting || deliveryStatus.deliverable === false}
              style={{
                background: deliveryStatus.deliverable
                  ? "var(--color-accent)"
